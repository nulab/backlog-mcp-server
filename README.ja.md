...
`--export-descriptions` フラグを指定してバイナリを実行することで、現在の説明（オーバーライドを含む）をエクスポートできます。このフラグは 0.15.x までは `--export-translations` という名前でした。旧名も当面は動作しますが、非推奨の警告を出力し、0.16.0 で削除されます。
...
- `--export-descriptions`: ツール一覧の構築時に解決される説明キーと値をエクスポート。旧名は `--export-translations` で、非推奨エイリアスとして当面動作しますが 0.16.0 で削除されます
