...
ここでいう説明は、モデルがどのツールを呼ぶか、引数に何を入れるかを判断するために読むテキストです。エンドユーザーには表示されません。したがってオーバーライドは、似た2つのツールを区別させたり、チームの運用ルールを添えたりしてツール選択を誘導するための手段であり、応答の言語を変えるためのものではありません。
...
文字列以外の値は無視され、組み込みのデフォルト値が使われます。
...
これにより、行ったカスタマイズを含む、オーバーライド可能なすべてのキーと現在の値が標準出力に出力されます。オーバーライドに使えるキー名を調べる用途に使ってください。
...
- `--export-translations`: オーバーライド可能なすべての説明キーと値をエクスポート
