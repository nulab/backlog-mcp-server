...
サーバーは stderr に JSON でログを出力します。既定のレベルは `error` です。
`LOG_LEVEL` に `trace` / `debug` / `info` / `warn` / `error` / `fatal` / `silent`
のいずれかを設定すると変更できます:
...
stdio トランスポートでは stdout に JSON-RPC が流れるため、ログが stdout に
出ることはありません。開発中に読みやすくしたい場合は、stderr を整形してください:
